...
module.exports = gali(tofn, (rule, data)  => rule(data));

function tofn(checker) {
  var checkerType = typeof(checker);
  if(checkerType == 'string') return getfn(checker);
  if(checkerType != 'object' ||checker == null)
    throw new Error('need string or not null object but got ' + checkerType);

  var checkers = Object.keys(checker).map(key => {
    return {
      name: key,
      fn: tofn(checker[key])
    };
  ...
  return function(data) {
    return Promise.all(checkers.map(c => c.fn(data[c.name], c.name, data)))
    .then(function(values) {
      var result = {};
      values.forEach(function(value, index) {
        result[checkers[index].name] = value;
      });
      return result;
    });
  };
...
function getConfig(type) {
  ...
  if(!res || (res[2] != ':' && res[3] != ''))
    throw new Error(`${type} is not a regular type string`);
  var config = {
